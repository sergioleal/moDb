...
int main() {
    ...
    modb::object::ObjectId customer_id{};
    {
        ...
        auto customer = tx ? database->create(*tx, Customer{"Ana", 42})
                           : modb::Result<modb::object::Handle<Customer>>{
                                 std::unexpected(tx.error())};
        if (!customer || !tx->commit()) {
            std::cerr << "failed to persist Customer\n";
            cleanup(path);
            return 1;
        }
    ...
    auto attached = modb::object::DatabaseRegistry::instance().attach(database);
    if (!database->bind(customer_binding())) {
        std::cerr << "failed to rebind Customer\n";
        cleanup(path);
        return 1;
    }
    auto value = database->materialize(*database->get<Customer>(customer_id));
    std::cout << value->name << " score=" << value->score << '\n';
    modb::object::DatabaseRegistry::instance().detach(*attached);
    cleanup(path);
    return 0;
}
