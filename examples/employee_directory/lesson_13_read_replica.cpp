...
// Builds on Lesson 12 (examples/employee_directory/lesson_12_async_io.cpp).
// See docs/training/en/13-read-replica.md.
...
// the Lesson 6 report, updated for Lesson 11's schema.
modb::Result<std::int64_t> payroll_total_v4(modb::object::Database& database,
...
int lesson_13_read_replica(const std::filesystem::path& path, const DirectoryIds&) {
    auto opened = modb::object::Database::open(path);
    if (!opened) {
        std::cerr << opened.error().message << '\n';
        return 1;
    ...
    if (!attached) {
        std::cerr << "failed to attach primary\n";
        return 1;
    }
    if (!primary->bind(employee_v4_binding()) || !primary->bind(department_binding()) ||
    ...
    // The follower is a separate file, next to the main one -- unlike every
    // earlier lesson, this is NOT the same continuously-reopened database.
    const auto temp_dir = path.parent_path() / (path.stem().string() + "-bootstrap-tmp");
    const auto follower_path = path.parent_path() / (path.stem().string() + "-replica.modb");
    ...
    std::cout << "Lesson 13: bootstrap snapshot cut at LSN " << snap->begin.cut_lsn << " ("
              << snap->begin.size_bytes << " bytes)\n";
    if (auto installed = modb::repl::install_bootstrap_snapshot(*snap, follower_path); !installed) {
        std::cerr << installed.error().message << '\n';
        return 1;
    }
    std::cout << "Lesson 13: installed the follower copy at " << follower_path.string() << '\n';
    ...
    std::cout << "Lesson 13: follower's database_uuid matches the primary's\n";
    (void)follower->set_read_only_replica(true);

    // Rerun Lesson 6's payroll report -- now against the follower.
    auto follower_snapshot = follower->snapshot();
    if (!follower_snapshot) {
        std::cerr << follower_snapshot.error().message << '\n';
        return 1;
    }
    auto total = payroll_total_v4(*follower, *follower_snapshot);
    if (!total) {
        std::cerr << total.error().message << '\n';
        return 1;
    }
    std::cout << "Lesson 13: payroll total from the follower = " << *total << '\n';
    ...
    std::cout << "Lesson 13: follower rejected a write attempt as expected: "
              << denied.error().message << '\n';

    modb::object::DatabaseRegistry::instance().detach(*follower_attached);
    modb::object::DatabaseRegistry::instance().detach(*attached);

    // The follower and its bootstrap scratch space are this lesson's own
    // artifacts, not part of the shared employee-directory file.
    std::error_code ignored;
...
int main() {
    ...
    if (const auto status = lesson_12_async_io(path, ids); status != 0) {
        cleanup(path);
        return status;
    }
    const auto status = lesson_13_read_replica(path, ids);
