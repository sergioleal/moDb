...
// Builds on Lesson 7 (examples/employee_directory/lesson_07_queries.cpp).
// See docs/training/en/08-networking.md.
...
int lesson_08_networking(const std::filesystem::path& path, const DirectoryIds&) {
    ...
    if (!server->database().bind(employee_v3_binding())) {
        std::cerr << "failed to bind Employee on the server\n";
        return 1;
    }
...
int main() {
    ...
    const auto status = lesson_08_networking(path, ids);
