...
CaseRunResult run_read_hotspot(const Case& c, const std::filesystem::path& work_dir,
    ...
    if (c.target != "embedded") {
        ...
        result.error = "read_hotspot: alvo '" + c.target +
    ...
    return run_read_hotspot_embedded(params, out_db_path);
}
