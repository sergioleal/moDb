...
CaseRunResult run_snapshot_hold(const Case& c, const std::filesystem::path& work_dir,
    ...
    if (c.target != "embedded") {
        ...
        result.error = "snapshot_hold: alvo '" + c.target +
    ...
    return run_snapshot_hold_embedded(params, out_db_path);
}
