...
CaseRunResult run_oversubscribed_churn(const Case& c, const std::filesystem::path& work_dir,
    ...
    if (c.target != "embedded") {
        ...
        result.error = "oversubscribed_churn: alvo '" + c.target +
    ...
    return run_oversubscribed_churn_embedded(params, out_db_path);
}
