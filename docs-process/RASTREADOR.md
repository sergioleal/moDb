...
| [16](#fase-16--catch-up-de-réplica-por-download-do-wal) | Catch-up de réplica por WAL baixável | ✅ Concluída | 8/8 | Fases 14, 15 · 16A–16E |
| **Total** | | | **135/173 (~78%)** | |
...
Status: ✅ Concluída (2/2) — commit `18f85d3`, tag `0.0.8f`.
...
Status: ✅ Concluída — merge `2a45e39`, tag `0.0.10b` (2026-07-19).
...
Status: ✅ Concluída — merge `2438c1b`, tag `0.0.10c` (2026-07-19).
...
Status: ✅ Concluída — merge `07ab67c`, tag `0.0.11a` (2026-07-19).
...
Status: ✅ Concluída — merge `ed0d661`, tag `0.0.11b` (2026-07-19).
...
Status: ✅ Concluída (12/12) — cinco entregas verticais 12A–12E.
Definição completa:
...
| 12.5 | BFS e DFS lazy/canceláveis sob snapshot | ✅ | 12C · tag `0.0.12c` |
| 12.6 | Caminho mínimo sem peso | ✅ | 12D · tag `0.0.12d` |
| 12.7 | Detecção de ciclo + ordenação topológica | ✅ | 12D · `graph_cycle` em ciclo |
| 12.8 | Componentes conexos | ✅ | 12D · View não direcionada explícita |
| 12.9 | Arestas de entrada por índice de `Ref` | ✅ | 12B · merge `dd2adb3`, tag `0.0.12b` |
| 12.10 | Testes de snapshot, reabertura, cancelamento, limites, órfãs e ownership | ✅ | 12E · reopen + algoritmos |
| 12.11 | CLI `graph bfs/dfs/shortest-path/toposort` | ✅ | 12E · tag `0.0.12e` |
| 12.12 | Benchmarks de topologia e cache | ✅ | 12E · `graph.traversal.warm/cold` |
...
Status: ✅ Concluída (8/8) — branch `codex/phase-16-wal-catchup`.
Definição completa:
...
| 16.2 | Estados persistentes da réplica (`empty` → `up_to_date`) | ✅ | 16A · `modb.replica_catchup_state` |
| 16.3 | Manifesto de WAL baixável com UUID/timeline/intervalos/hashes | ✅ | 16B · `modb.replica_wal_manifest` |
| 16.4 | Validação de cobertura local, gap e retenção | ✅ | 16B · `bootstrap_required` quando o WAL não cobre o buraco |
| 16.5 | Downloader com spool durável, fsync, rename atômico e retomada | ✅ | 16C · `modb.replica_wal_download` |
| 16.6 | Apply batch reutilizando o applier idempotente da Fase 14 | ✅ | 16D · `modb.replica_catchup_apply` |
| 16.7 | CLI/status `modb replicate catch-up` | ✅ | 16E · status com `catchup_state`, `target_lsn`, lag |
| 16.8 | Docs operacionais e suítes verdes | ✅ | 16E · `OPERACAO_REPLICACAO.md` |
