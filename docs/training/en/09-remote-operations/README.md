# Lesson 9 — Remote Operations: Transferring an Employee — Build

This folder is self-contained: the lesson doc, its source, and this build
guide all live together.

- Lesson doc: [09-remote-operations.md](09-remote-operations.md)
- Source: [lesson_09_remote_operations.cpp](lesson_09_remote_operations.cpp)

## Build

From the repository root, with the `debug` preset already configured
(`cmake --preset debug`, once per clone):

```powershell
cmake --build --preset debug --target employee_directory_lesson_09
```

## Run

```powershell
.\build\debug\employee_directory_lesson_09.exe
```

This lesson only contains its own new code — it opens the persistent
database file Lessons 1-8 built up and continues from there, so run
Lessons 1-8 first, in order. See
[../README.md](../README.md#how-the-code-is-organized) for how the chain
works.
