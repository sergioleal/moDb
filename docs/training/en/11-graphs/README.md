# Lesson 11 — The Org Chart: Graphs — Build

This folder is self-contained: the lesson doc, its source, and this build
guide all live together.

- Lesson doc: [11-graphs.md](11-graphs.md)
- Source: [lesson_11_graphs.cpp](lesson_11_graphs.cpp)

## Build

From the repository root, with the `debug` preset already configured
(`cmake --preset debug`, once per clone):

```powershell
cmake --build --preset debug --target employee_directory_lesson_11
```

## Run

```powershell
.\build\debug\employee_directory_lesson_11.exe
```

This lesson only contains its own new code — it opens the persistent
database file Lessons 1-10 built up and continues from there, so run
Lessons 1-10 first, in order. See
[../README.md](../README.md#how-the-code-is-organized) for how the chain
works.
